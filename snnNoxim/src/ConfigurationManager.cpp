...
void loadConfiguration() {
    ...
    GlobalParams::network_type = config["network_type"].as<string>();
    GlobalParams::rout_only_nodes = config["rout_only_nodes"].as<vector <int>>();
    GlobalParams::dim_x = config["dim_x"].as<int>();
    GlobalParams::dim_y = config["dim_y"].as<int>();
    ...
    GlobalParams::neural_network_filename = config["neural_network_filename"].as<string>();
    //std::cout << "neural_network_filename: " << GlobalParams::neural_network_filename << endl;
    
    GlobalParams::spike_step = config["spike_step"].as<int>();
...
void showConfig()
{
    cout << "Using the following configuration: " << endl
         ...
         << "- dim_x = " << GlobalParams::dim_x << endl
         << "- dim_y = " << GlobalParams::dim_y << endl
         ...
         << "- routing_filename = " << GlobalParams::routing_filename << endl
...
{
    if (GlobalParams::dim_x <= 1) {
	cerr << "Error: dimx must be greater than 1" << endl;
	exit(1);
    }

    if (GlobalParams::dim_y <= 1) {
    ...
    else
    {
        for (int i = 1; i < arg_num; i++)
        {
            ...
            else if (!strcmp(arg_vet[i], "-log"))
            {
                ...
                GlobalParams::dim_x = atoi(arg_vet[++i]);
            else if (!strcmp(arg_vet[i], "-dimy"))
                GlobalParams::dim_y = atoi(arg_vet[++i]);
            ...
            else if (!strcmp(arg_vet[i], "-routing"))
            {
                ...
                else if ((GlobalParams::routing_algorithm == ROUTING_TABLE_BASED) || (GlobalParams::routing_algorithm == ROUTING_FILE_BASED))
                {
                    GlobalParams::routing_filename = arg_vet[++i];
            ...
            else if (!strcmp(arg_vet[i], "-traffic"))
            {
                ...
                if (!strcmp(traffic, "random")) GlobalParams::traffic_distribution = TRAFFIC_RANDOM;
                    ...
                    GlobalParams::traffic_distribution =
                            TRAFFIC_TABLE_BASED;
                    GlobalParams::traffic_filename = arg_vet[++i];
            ...
            else if (!strcmp(arg_vet[i], "-neuron_node"))
                GlobalParams::neuron_node_filename = arg_vet[++i];
            else if (!strcmp(arg_vet[i], "-neural_network"))
                GlobalParams::neural_network_filename = arg_vet[++i];
